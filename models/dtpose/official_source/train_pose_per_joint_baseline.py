...
if __name__ == '__main__':
    ...
    parser.add_argument("--config_file", type=str, help="Configuration YAML file", default='config/mmfi/pose_config-Copy1.yaml')
    parser.add_argument("--seed", type=int, default=None)
    parser.add_argument("--dataset_root", type=str, default=None)
    parser.add_argument("--run_root", type=str, default=None)
    parser.add_argument("--save_epoch_features", action="store_true")
    ...
    run_root = args.run_root or os.path.join(
        'p2s2_per_joint_runs', config['experiment_name'], f"seed_{config['seed']:02d}"
    )
    run_root = os.path.abspath(run_root)
    os.makedirs(run_root, exist_ok=True)
    config['save_path'] = os.path.join(run_root, 'weights')
    ...
    if config['pretrained_model_path'] is not None:
        ...
        writer = SummaryWriter(os.path.join(run_root, 'tensorboard'))
        if config['dataset_name'] == 'mmfi-csi':
            model = ViT_Pose_Decoder(model.encoder, keypoints=17, coor_num=3, token_num=114, dataset=config['dataset_name']).to(device)   # 72*5
            optim = torch.optim.SGD(filter(lambda p:p.requires_grad, model.parameters()),lr=8.49e-04, weight_decay=0.01)   #  weight_decay=0.01
        elif config['dataset_name'] == 'person-in-wifi-3d':
        ...
        # save train feature
        train_feature_path = os.path.join(run_root, 'features', 'train')
        if not os.path.exists(train_feature_path):
            os.makedirs(train_feature_path)
        # save test feature
        test_feature_path = os.path.join(run_root, 'features', 'validation')
        ...
        if config['dataset_name'] == 'mmfi-csi':
            if config['experiment_name'] == 'metafi':
                ...
                optim = torch.optim.AdamW(model.parameters(), lr=8.49e-04)
        elif config['dataset_name'] == 'person-in-wifi-3d':
            if config['experiment_name'] == 'metafi':
    ...
    for epoch in range(config['total_epoch']):
        ...
        with torch.no_grad():
            ...
            for batch_idx, batch_data in enumerate(val_loader):
                ...
                pampjpe_joints_list.append(pampjpe_joints)
                losses.append(loss.item())
            ...
            gt_list = np.concatenate(gt_list, axis=0)
            pred_list = np.concatenate(pred_list, axis=0)
            mpjpe_joints_all = np.concatenate(mpjpe_joints_list, axis=0)
            pampjpe_joints_all = np.concatenate(pampjpe_joints_list, axis=0)
            feature_list = np.concatenate(feature_list, axis=0)
            wifi_list = np.concatenate(wifi_list, axis=0)
            if config['dataset_name'] == 'mmfi-csi' or config['dataset_name'] == 'wipose':
                label_list = np.concatenate(label_list, axis=0)
       ...
       # 以 MPJPE 为唯一主指标进行判断
        if avg_val_mpjpe < best_val_mpjpe:
            ...
            np.savez_compressed(
                os.path.join(weights_path, 'per_joint_validation.npz'),
                seed=np.asarray(config['seed']),
                epoch=np.asarray(epoch),
                mpjpe_mm=mpjpe_joints_all * 1000.0,
                pa_mpjpe_mm=pampjpe_joints_all * 1000.0,
                pred_pose=pred_list,
                gt_pose=gt_list,
            )
            
            # 2. 用此时此刻的 PA-MPJPE 覆盖历史记录
            # 注意：这里的 Best pa-mpjpe 表示“MPJPE 最优 epoch 对应的 PA-MPJPE”
            best_val_pampjpe = avg_val_pampjpe
            torch.save(model, '{}/pose_pampjpe.pt'.format(weights_path))
            
            # 3. 同步记录这个 epoch 对应的 PCK
            ...
            print(f'---> 同步更新了 Epoch {epoch} 的所有指标 <---')
        """
        ''' save model '''
        # save mpjpe pampjpe
        if avg_val_mpjpe < best_val_mpjpe:
