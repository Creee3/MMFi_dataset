...
每个组合: 5 seeds × 30 epochs, 使用 Bayesian HPO 最优参数

用法:
  python run_cmc_mixup_all_splits.py <dataset_root> <config_file>
...
SEEDS = list(range(5))
RESULTS_ROOT = "experiment_results/cmc_mixup_all_splits"
...
def collect_results():
    ...
    print("\n" + "=" * 130)
    print("📊 CMC + Mixup 跨 Protocol × Split 对比")
    print(f"   Best hparams: lr=1.03e-04  bs=64  no CBAM  |  n={len(SEEDS)}")
    print("=" * 130)
    header = (
        f"{'Protocol':<12s} {'Split':<20s} "
        f"{'PA-MPJPE (mm)':<20s} {'MPJPE (mm)':<20s} "
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="CMC+Mixup across all Protocol×Split combinations")
    ...
    print("=" * 72)
    print("🔬 CMC + Mixup — 跨 3 Protocol × 3 Split 实验")
