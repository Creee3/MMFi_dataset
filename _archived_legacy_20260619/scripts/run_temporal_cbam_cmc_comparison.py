...
对比两组结果：
1. 时序 WiFi + CBAM（baseline）
2. 时序 WiFi + CBAM + CMC（训练时使用 RGB 2D pose 作为特权信息，不做 distillation）

设计目标：
- 在 run_temporal_only_cmc_comparison.py 的基础上仅启用 CBAM
- 统一保存到 experiment_results/temporal_cbam_cmc_comparison/
- 每个 variant 支持多 seed 运行
- 自动把 strict_offline_runs 下最新一次训练目录移动到标准结果目录
- 自动汇总 summary.json

用法：
  python run_temporal_cbam_cmc_comparison.py <dataset_root> <config_file>
例如：
  python run_temporal_cbam_cmc_comparison.py data_base config.yaml
"""
...
SEEDS = list(range(10))
RESULTS_ROOT = "experiment_results/temporal_cbam_cmc_comparison_lr1e-3_bs32"
...
VARIANTS = {
    "A_temporal_wifi_cbam": {
        "label": "Temporal WiFi + CBAM",
        "entry": "train_baseline.py",
        "prefix": "temporal_baseline_cbam",
        "extra_args": [],
    },
    "B_temporal_wifi_cbam_cmc": {
        "label": "Temporal WiFi + CBAM + CMC",
...
def collect_results():
    ...
    print("\n" + "=" * 120)
    print("📊 Temporal WiFi + CBAM  vs  Temporal WiFi + CBAM + CMC")
    ...
    if all(k in all_results for k in order):
        base = all_results["A_temporal_wifi_cbam"]["pa_mpjpe_mean"]
        cmc = all_results["B_temporal_wifi_cbam_cmc"]["pa_mpjpe_mean"]
        improve = base - cmc
...
def main():
    ...
    if args.variant == "baseline":
        order = ["A_temporal_wifi_cbam"]
    elif args.variant == "cmc":
        order = ["B_temporal_wifi_cbam_cmc"]
    else:
        order = [
            "A_temporal_wifi_cbam",
            "B_temporal_wifi_cbam_cmc",
