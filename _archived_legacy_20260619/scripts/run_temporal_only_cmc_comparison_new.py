"""
run_temporal_only_cmc_comparison.py

对比两组结果（在原 run_temporal_wifi_cmc_comparison.py 基础上仅关闭训练技巧）：
1. 只有时序的 WiFi baseline
2. 时序 + CMC（训练时使用 RGB 2D pose 作为特权信息，但不做 distillation）

设计目标：
- 尽可能复用现有训练逻辑与文件组织形式
- 统一保存到 experiment_results/temporal_only_cmc_comparison/
- 每个 variant 支持多 seed 运行
- 自动把 strict_offline_runs 下最新一次训练目录移动到标准结果目录
- 自动汇总 summary.json

用法：
  python run_temporal_only_cmc_comparison.py <dataset_root> <config_file>
例如：
  python run_temporal_only_cmc_comparison_new.py data_base config.yaml
"""
...
SEEDS = list(range(10))
RESULTS_ROOT = "experiment_results/temporal_only_cmc_comparison_lr1e-3_bs32"
...
VARIANTS = {
    "A_temporal_wifi_baseline": {
        "label": "Temporal WiFi Baseline",
...
def collect_results():
    ...
    print("\n" + "=" * 120)
    print("📊 Temporal WiFi Baseline vs Temporal WiFi + CMC")
    print("   cross_subject_split / Protocol 2 / no root-relative / n=10")
    print("=" * 120)
    print(
        f"{'Variant':<26s} {'Description':<28s} {'PA-MPJPE (mm)':<20s} "
    ...
    for vname in order:
        if vname not in all_results:
            print(f"{vname:<26s} {'[missing]':<28s}")
            continue
        r = all_results[vname]
        print(
            f"{vname:<26s} {r['label']:<28s} {r['PA-MPJPE']:<20s} "
    ...
    if all(k in all_results for k in order):
        base = all_results["A_temporal_wifi_baseline"]["pa_mpjpe_mean"]
        cmc = all_results["B_temporal_wifi_cmc"]["pa_mpjpe_mean"]
        improve = base - cmc
...
def main():
    ...
    if args.variant == "baseline":
        order = ["A_temporal_wifi_baseline"]
    elif args.variant == "cmc":
        order = ["B_temporal_wifi_cmc"]
    else:
        order = [
            "A_temporal_wifi_baseline",
            "B_temporal_wifi_cmc",
