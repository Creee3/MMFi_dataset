...
对比两组结果：
1. 时序 WiFi + CBAM + Mixup（baseline）
2. 时序 WiFi + CBAM + Mixup + CMC（训练时使用 RGB 2D pose 作为特权信息，不做 distillation）

设计目标：
- 在 run_temporal_only_cmc_comparison.py 的基础上同时启用 CBAM 与 Mixup
- 统一保存到 experiment_results/temporal_cbam_mixup_cmc_comparison/
- 每个 variant 支持多 seed 运行
- 自动把 strict_offline_runs 下最新一次训练目录移动到标准结果目录
- 自动汇总 summary.json

用法：
  python run_temporal_cbam_mixup_cmc_comparison.py <dataset_root> <config_file>
例如:
  python run_temporal_cbam_mixup_cmc_comparison.py data_base config.yaml
"""
...
SEEDS = list(range(10))
RESULTS_ROOT = "experiment_results/temporal_cbam_mixup_cmc_comparison_lr1e-3_bs32"
...
COMMON_ARGS = [
    ...
    "--mixup_alpha", "0.4",
    # 不再传 --no_cbam，使用 add_common_args 中默认的 use_cbam=True
    "--lr", "1e-3",
...
VARIANTS = {
    "A_temporal_wifi_cbam_mixup": {
        "label": "Temporal WiFi + CBAM + Mixup",
        "entry": "train_baseline.py",
        "prefix": "temporal_baseline_cbam",
        "extra_args": [],
    },
    "B_temporal_wifi_cbam_mixup_cmc": {
        "label": "Temporal WiFi + CBAM + Mixup + CMC",
...
def collect_results():
    ...
    print("\n" + "=" * 120)
    print("📊 Temporal WiFi + CBAM + Mixup  vs  Temporal WiFi + CBAM + Mixup + CMC")
    print("   cross_subject_split / Protocol 2 / no root-relative / n=10")
    print("=" * 120)
    print(
        f"{'Variant':<34s} {'Description':<38s} {'PA-MPJPE (mm)':<20s} "
    ...
    for vname in order:
        if vname not in all_results:
            print(f"{vname:<34s} {'[missing]':<38s}")
            continue
        r = all_results[vname]
        print(
            f"{vname:<34s} {r['label']:<38s} {r['PA-MPJPE']:<20s} "
    ...
    if all(k in all_results for k in order):
        base = all_results["A_temporal_wifi_cbam_mixup"]["pa_mpjpe_mean"]
        cmc = all_results["B_temporal_wifi_cbam_mixup_cmc"]["pa_mpjpe_mean"]
        improve = base - cmc
...
def main():
    ...
    if args.variant == "baseline":
        order = ["A_temporal_wifi_cbam_mixup"]
    elif args.variant == "cmc":
        order = ["B_temporal_wifi_cbam_mixup_cmc"]
    else:
        order = [
            "A_temporal_wifi_cbam_mixup",
            "B_temporal_wifi_cbam_mixup_cmc",
