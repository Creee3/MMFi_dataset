...
try:
    import cv2
except ImportError:  # CSI-only runs do not read depth frames.
    cv2 = None
import torch
import numpy as np
...
def _action_segments_csv_path():
    """Find the auxiliary action-segment table beside this code package.

    The archived trainer used a Linux-only absolute path.  Keeping the
    fallback preserves compatibility with the original repository while
    allowing the same code bundle to run on the Windows 5090 machine.
    """
    package_path = os.path.join(
        os.path.dirname(os.path.dirname(os.path.abspath(__file__))),
        'MMFi_action_segments.csv',
    )
    legacy_path = '/root/autodl-tmp/DT-Pose-main/MMFi_action_segments.csv'
    for path in (package_path, legacy_path):
        if os.path.isfile(path):
            return path
    raise FileNotFoundError(
        'MMFi_action_segments.csv was not found beside the code package '
        f'or at the legacy path: {legacy_path}'
    )
...
class MMFi_Dataset(Dataset):
    def __init__(
        ...
        self.include_next_frame = bool(include_next_frame)
        ...
        if self.frame_start is not None:
            if self.frame_start < 0 or self.frame_end < self.frame_start:
                raise ValueError("Invalid frame_start/frame_end.")
            if self.frame_end >= 297:
                raise ValueError("MM-Fi frame indices must be in [0, 296].")
            if self.include_next_frame and self.frame_end >= 296:
                raise ValueError(
                    "The next-frame loader cannot include target frame 296; "
                    "set include_next_frame=false for the full target range."
                )
    ...
    def load_test_iid_data(self):
        ...
        seg_csv_path = _action_segments_csv_path()
    ...
    #     data_info = []
    #     # read mmfi_action_segments.csv
    #     seg_csv_path = '/root/autodl-tmp/DT-Pose-main/MMFi_action_segments.csv'
    #     seg_csv = pd.read_csv(seg_csv_path)
    #     # 初始化字典
    #     action_dict = defaultdict(lambda: {'start': [], 'end': []})
    #     # 解析并填充字典
        ...
        if self.data_unit == 'sequence':
            ...
            for mod in item['modality']:
                ...
                if os.path.isfile(data_path):
                    ...
                    sample['input_'+mod] = data_mod
                # The historical baseline loader read a next frame for an
                # unused training path.  The equal-sample rerun disables it
                # so target frame 296 can be retained.
                if self.include_next_frame and self.split == 'training':
...
def make_dataset(semi, dataset_root, config):
    ...
    dataset_options = {
        'frame_start': config.get('target_frame_start'),
        'frame_end': config.get('target_frame_end'),
        'include_next_frame': config.get('include_next_frame', True),
    }
    train_dataset = MMFi_Dataset(
        semi, database, config['data_unit'],
        **config_dataset['train_dataset'], **dataset_options
    )
    val_dataset = MMFi_Dataset(
        semi, database, config['data_unit'],
        **config_dataset['val_dataset'], **dataset_options
    )
...
def collate_fn_padd(batch):
    ...
    if (
        batch_data['split'][0] == 'training'
        and 'input_' + mod + '_next_frame' in batch[0]
    ):
        _input_next_frame = [
            np.array(sample['input_' + mod + '_next_frame']) for sample in batch
        ]
