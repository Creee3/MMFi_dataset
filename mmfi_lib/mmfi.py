...
try:
    ...
    cv2 = None
    print('[WARN] OpenCV (cv2) not installed. Depth modality will be unavailable.')
import torch
import numpy as np
...
class MMFi_Dataset(Dataset):
    ...
    def read_dir(self, dir):
        ...
        if mod in ['infra1', 'infra2', 'rgb']:  # rgb, infra1 and infra2 are 2D keypoints
            ...
            if cv2 is None:
                raise ImportError('Depth modality requires OpenCV (cv2), but it is not installed.')
...
def make_dataloader(dataset, is_training, generator, batch_size, collate_fn_padd=collate_fn_padd, num_workers=8):
    loader_kwargs = dict(
        dataset=dataset,
