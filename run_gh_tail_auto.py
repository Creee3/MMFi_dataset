...
RESULTS_ROOT = Path("experiment_results/gh_tail")
EXPECTED_TAG = (
    "baseline_lr7.0e-05_bs64_a0p16_do0p15_ly2_ff1024_head512"
    "_an0p02_af0p05_at0p05_rel0p0_root0p0_sub0p0"
    "_bone0p005_phgraph_root_tail5_tailsoup"
)
...
def find_run_dir(results_root, seed):
    candidates = []
...
def main():
    parser = argparse.ArgumentParser(
        description="Run the focused graph-head tail-soup experiment and compare checkpoints."
    )
    ...
    for seed in args.seeds:
        run_dir = find_run_dir(RESULTS_ROOT, seed)
        if run_dir is None:
            raise SystemExit(
    ...
    summarize_checkpoint_results(
        run_dirs,
        RESULTS_ROOT / "tail_checkpoint_summary.json",
    )
